    ...
    try {
      ...
      if (selectedImage) {
        ...
        if (!imgRes.ok || !imgJson.url) {
          clearInterval(progressTimer);
          throw new Error(imgJson.error || 'Image upload failed');
        }
      ...
      // ── Step 3: Insert metadata into Supabase DB ──
      ...
      if (!error) {
        ...
        setTimeout(() => setUploadStep(5), 400);
              ...
              {uploadStep === 4 && (
                <div className="text-center py-20">
                  <div className="relative w-28 h-28 mx-auto mb-8">
                     <svg className="w-full h-full transform -rotate-90 drop-shadow-[0_0_15px_rgba(234,179,8,0.5)]" viewBox="0 0 100 100">
                       <circle cx="50" cy="50" r="45" fill="none" stroke="rgba(255,255,255,0.05)" strokeWidth="8" />
                       <circle 
                         cx="50" cy="50" r="45" fill="none" stroke="#eab308" strokeWidth="8" strokeLinecap="round"
                         strokeDasharray={`${uploadProgress * 2.83} 283`} className="transition-all duration-300 ease-out" 
                       />
                     </svg>
                     <div className="absolute inset-0 flex items-center justify-center flex-col">
                       <span className="font-black text-2xl text-gold-500">{uploadProgress}%</span>
                     </div>
                  </div>
                  <h2 className="text-xl font-bold mb-2">
                    {uploadProgress < 95 ? "Syncing to Cloudflare R2..." : "Finalizing Checksums..."}
                  </h2>
                  <p className="text-gray-400 max-w-sm mx-auto">Please do not close this window. Safely pushing chunks over encrypted pipeline.</p>
                </div>
              )}
